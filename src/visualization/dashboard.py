"""Dashboard Generator for Tableau Integration"""
...
class DashboardGenerator:
    """Generate dashboard data and visualizations. Tracks DAU/MAU, retention, skip rates."""
    
    def __init__(self, output_dir: str = "dashboards"):
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(parents=True, exist_ok=True)
    
    def calculate_dau_mau(self, sessions: pd.DataFrame, date_column: str = 'timestamp') -> pd.DataFrame:
    ...
    def calculate_skip_rates(self, sessions: pd.DataFrame, tracks: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    def calculate_retention_curve(self, users: pd.DataFrame, sessions: pd.DataFrame, days: int = 30) -> pd.DataFrame:
        users = users.copy()
        sessions = sessions.copy()
        ...
        sessions_with_signup['days_since_signup'] = (pd.to_datetime(sessions_with_signup['session_date']) - sessions_with_signup['signup_date']).dt.days
        
        total_users = users['user_id'].nunique()
        retention = []
        for day in range(days + 1):
            active = sessions_with_signup[sessions_with_signup['days_since_signup'] == day]['user_id'].nunique()
            retention.append({'day': day, 'users': active, 'retention_rate': active / total_users})
        
        return pd.DataFrame(retention)
    
    def export_for_tableau(self, dau_mau: pd.DataFrame, skip_rates: Dict, retention: pd.DataFrame) -> None:
