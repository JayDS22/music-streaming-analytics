"""A/B Testing Framework with Statistical Significance Testing (p<0.05)"""
...
class ABTestFramework:
    ...
    def calculate_sample_size(self, baseline_rate: float, mde: float, power: float = 0.80) -> int:
        ...
        if effect == 0:
            return 10000
        return int(np.ceil(variance * (z_alpha + z_beta) ** 2 / effect ** 2))
    
    def create_experiment(self, name: str, control_users: List[str], treatment_users: List[str], description: str = "") -> None:
    ...
    def assign_users_randomly(self, user_ids: List[str], treatment_ratio: float = 0.5, seed: int = 42) -> Tuple[List[str], List[str]]:
    ...
    def analyze_results(self, experiment_name: str, metric_data: pd.DataFrame, metric_column: str, user_column: str = 'user_id') -> ExperimentResults:
...
def run_ab_test_simulation(n_users: int = 10000, effect: float = 0.05) -> ExperimentResults:
    """Run simulated A/B test for demonstration."""
    np.random.seed(42)
    ...
    ab = ABTestFramework()
    ab.create_experiment("test", [f"u{i}" for i in range(control_n)], [f"u{i}" for i in range(control_n, n_users)])
    
    data = pd.DataFrame({'user_id': [f"u{i}" for i in range(n_users)], 'rate': np.concatenate([control, treatment])})
